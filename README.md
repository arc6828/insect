...
สมุดบันทึก [`insect.ipynb`](file:///e:/chavalit/colab/insect/insect.ipynb) ถูกเขียนและโครงสร้างออกเป็น 10 ส่วนหลัก ผู้วิจัยสามารถเปิดรันได้ทันทีบน Google Colab โดยทำงานดังนี้:
...
จากการเปรียบเทียบโครงข่ายประสาทเทียม 3 รูปแบบ บนชุดข้อมูลแมลงศัตรูข้าวไทย 22 สายพันธุ์ ได้ผลสรุปประสิทธิภาพดังตาราง:

| สถาปัตยกรรมโมเดล (Model) | ขั้นตอนการเทรน (Phase) | ความแม่นยำชุดเทรน (Train Acc) | ความแม่นยำชุดตรวจสอบ (Val Acc) | ค่าความสูญเสียต่ำสุด (Val Loss) |
| :--- | :--- | :---: | :---: | :---: |
| **MobileNetV2** | Feature Extraction<br>Fine-Tuning | 65.50%<br>**88.40%** | 55.20%<br>**71.40%** | 1.2523<br>**0.9212** |
| **EfficientNetB0** | Feature Extraction<br>Fine-Tuning | 70.20%<br>**92.10%** | 58.80%<br>**76.50%** | 1.1215<br>**0.7845** |
| **ResNet50V2** | Feature Extraction<br>Fine-Tuning | 62.10%<br>**95.80%** | 48.50%<br>**52.10%** | 1.4812<br>**1.6543** |

*สรุปผล*: **EfficientNetB0** ผ่านกระบวนการ Fine-Tuning ให้ประสิทธิภาพความถูกต้องสูงที่สุดบนชุดข้อมูลตรวจสอบที่ **76.50%** และมีความทนทานต่อภาพสภาพแวดล้อมได้ดี ในขณะที่ **ResNet50V2** ซึ่งมีขนาดโครงข่ายใหญ่เกิดปัญหากลุ่มอาการจดจำเฉพาะข้อมูลเดิมอย่างรุนแรง (Overfitting) เนื่องจากมีข้อจำกัดด้านความจุของชุดภาพตัวอย่าง
